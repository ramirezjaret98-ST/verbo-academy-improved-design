...
export interface ExtSession {
  ...
  /** 2026-08-14: timestamp of the FIRST time the student/teacher clicked
   *  their own existing "Connect"/"Join Live Session" button for this
   *  session (see logSessionConnect below). This is a click, not confirmed
   *  Teams presence — there is no Microsoft Graph API integration, and none
   *  is planned (see decision_teams_link_sustitucion memory). Written via
   *  the `log_session_connect` RPC, never a direct column update, so it
   *  stays correct under `sessions_write_update` RLS (students can't
...
function mapSessionRow(row: SessionRow, members: MemberStatusRow[], edits: AdminEditRow[]): ExtSession {
  ...
  return {
    ...
    student_connected_at: row.student_connected_at ?? undefined,
    teacher_connected_at: row.teacher_connected_at ?? undefined,
 ...
 *  existing "Connect"/"Join Live Session" button for this session — a
 *  lightweight audit signal for Admin (Calendar/Overview), NOT real
 *  Microsoft Teams join/leave data (no Graph API integration, see
 *  decision_teams_link_sustitucion memory). Routed through the
 *  `log_session_connect` RPC (SECURITY DEFINER) rather than a direct
 *  column update: `sessions_write_update` RLS only allows admin/the owning
 *  teacher, so a student calling `updateSession` here would silently
 *  affect 0 rows exactly like the bug `studentSetSessionStatus` above
 *  already had to route around. First click per session/role wins
 *  (`coalesce` server-side) — a second click is a no-op both locally and
 *  server-side, so this is safe to call on every click without checking
 *  local state first. Optimistic, like every other mutator in this file;
 *  fire-and-forget from the caller's perspective (never blocks opening the
 *  video link — that navigation happens via the anchor's own `href`,
 *  unrelated to this call). */
export function logSessionConnect(sessionId: string, role: "student" | "teacher") {
  const prev = sessionsCache.find((s) => s.id === sessionId);
  const field = role === "student" ? "student_connected_at" : "teacher_connected_at";
  if (prev && prev[field]) return; // already logged locally, skip the round-trip
  const nowIso = new Date().toISOString();
  sessionsCache = sessionsCache.map((s) => (s.id === sessionId ? { ...s, [field]: s[field] ?? nowIso } : s));
  notify();

  const numericId = Number(sessionId);
  if (!Number.isFinite(numericId)) return;
  void (async () => {
    const { error } = await supabase.rpc("log_session_connect", {
      p_session_id: numericId,
      p_role: role,
    });
    if (error) {
      console.error("[sessions-store] log_session_connect failed", error);
      // No rollback: worst case the local optimistic timestamp is a few
      // seconds off from what the server would have recorded, or (if the
      // RPC's ownership check failed) never confirmed server-side at all —
      // either way, harmless for a best-effort audit signal, and Realtime
      // will correct the cache from the real row on the next sessions change.
    }
  })();
}
