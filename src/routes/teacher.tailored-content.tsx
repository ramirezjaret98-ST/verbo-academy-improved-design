// Teacher > Tailored Content — READ ONLY since the 2026-08-12 permission
// revert: Admin manages all Tailored content (units, video, files, course
// card, activities) at /admin/tailored-content. Teacher keeps this view to
// plan sessions. Note: this view now also gets an "Activities" button, which
// the old teacher-editable version never had — Tailored units previously had
// no way at all to see their activities.
...
import {
  tailoredUnitsForStudent, subscribeTailoredUnits, subscribeTailoredUnitCompletion, tailoredUnitDoneMap,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Personalized units for your Elite students — view only. Admin manages the content.
        </p>
      </div>
...
function StudentView({ studentId, studentName, onBack }: {
  ...
  const doneMap = useMemo(() => tailoredUnitDoneMap(), [studentId, rev]);
  const sessions = useMemo(() => loadSessions(), [rev]);
  const courseMeta = useMemo(() => courseMetaFor("tailored", studentId), [studentId, rev]);
  const doneCount = units.filter((u) => doneMap[u.id]).length;
      ...
      <Card className="!p-0">
        {units.length === 0 && (
          <div className="px-6 py-10 text-center text-sm text-muted-foreground">
            No units yet — Admin hasn't built this content.
          </div>
        )}
        {units.map((u, i) => {
