...
// for a new student/teacher, so beta users get a working login instead of
// the old localStorage-only "registration" that never touched auth.users.
//
...
// existing user-id-bridge.ts resolves it) and `must_change_password`.
//
...
Deno.serve(async (req: Request) => {
  ...
  let body: { legacyId?: string; email?: string; password?: string; name?: string; role?: string };
  try {
    body = await req.json();
  } catch {
    return new Response(JSON.stringify({ error: "Invalid JSON body" }), { status: 400, headers: { "Content-Type": "application/json" } });
  }
  const { legacyId, email, password, name, role } = body;
  if (!legacyId || !email || !password || !name || (role !== "student" && role !== "teacher")) {
    return new Response(JSON.stringify({ error: "legacyId, email, password, name and role ('student'|'teacher') are required" }), { status: 400, headers: { "Content-Type": "application/json" } });
  }
  ...
  const { error: updateErr } = await admin
    .from("app_users")
    .update({ legacy_id: legacyId, must_change_password: true })
